// tb_rv64i_uart_tx.sv — RV_NoPipelineCache system-level UART TX test
// Firmware sends "Hello Rocker" over UART0; TB decodes the serial stream and verifies.
// Pass: all 12 bytes match expected ASCII. GPIO[7:0]=0x55 confirms firmware completed.
`timescale 1ns/1ps
...
module tb_rv64i ();
  ...
  // ── Decode one 8N1 frame from uart0_tx_s (self-synchronising) ────────────
  task automatic tx_recv_8n1(output logic [7:0] data, output logic ferr);
    int i;
    ...
    #BIT_PERIOD;
    if (uart0_tx_s !== 1'b1) ferr = 1'b1;
  endtask

  // ── Primary check: decode "Hello Rocker" from serial TX ──────────────────
  initial begin
    expected[0]  = 8'h48; // H
    expected[1]  = 8'h65; // e
    expected[2]  = 8'h6C; // l
  ...
  initial begin
    #500_000_000;
    $display("WATCHDOG: 500 ms timeout — uart_tx");
