...
// Pipeline hazard testbench — functional check + cycle-count statistics.
//
// Plusargs:
//   +TEST_NAME=<name>      label written to error.txt
//   +EXPECTED=<int>        GPIO byte value that signals test pass
//   +INSTR_COUNT=<int>     number of dynamic instructions (0 = skip CPI output)
//
// On pass: error.txt receives  "<date> - <name>: Test ok (cycles=N, instr=M)"
// $display additionally prints the cycle count and CPI for waveform / log inspection.
...
module tb_rv64i ();
  ...
  initial begin
    ...
    if (!$value$plusargs("INSTR_COUNT=%0d",instr_count)) instr_count = 0;
    cs_count = 0;
  end
  ...
  // ── Cycle counter (counts CPU-clock cycles = clk_div_s = clk_div_o) ──────
  longint unsigned cycle_count;
  longint unsigned rst_deassert_cycle;

  always @(posedge clk_div_s) cycle_count <= cycle_count + 1;
  initial cycle_count = 0;

  // Capture the cycle number at the moment reset deasserts.
  always @(negedge rst_s) rst_deassert_cycle = cycle_count;
  ...
  //------------------------------------------
  // Check results on every cs_o rising edge; report statistics on pass.
  //------------------------------------------
  always @(posedge cs_s) begin
    automatic longint unsigned exec_cycles;
    #1;  // let gpio_io settle (same NBA region as cs_o)
    cs_count++;
    exec_cycles = cycle_count - rst_deassert_cycle;
    $display("CS #%0d: gpio=0x%0h  expected=0x%0h  cycles_since_rst=%0d",
             cs_count, gpio_s, expected, exec_cycles);
    if (gpio_s == expected) begin
      $display("Simulation %s succeeded", test_name);
      if (instr_count > 0)
        $display("STAT: test=%-24s  cycles=%0d  instr=%0d  CPI=%0.2f",
                 test_name, exec_cycles, instr_count,
                 real'(exec_cycles) / real'(instr_count));
      else
        $display("STAT: test=%-24s  cycles=%0d", test_name, exec_cycles);
      #100; #(1*2*clk_2_t);
      $fdisplay(fd, "%s - %s: Test ok (cycles=%0d, instr=%0d)",
                get_time(), test_name, exec_cycles, instr_count);
      $fclose(fd); $stop;
