...
module tb_rv64i ();
  ...
  begin
    ...
    assign imScanData_s = 32'h01D00513; // here is the check, the compiled data contains 1E, not 1D
    jtag_shift_in_imdr_to_rti(tclk_2_t, {imScanAddr_s, imScanData_s, 1'b1});
  ...
  begin
    ...
    begin
      $display("CS detected");
      case(gpio_s)
          29      : begin $display("Instr 99 - lui - load I-Mem: 0x%0h", gpio_s);  
                          $display("Simulation Load I-Mem succeeded"); #100; #(1*2*clk_2_t); 
                          $fdisplay(fd,"%s - Load I-Mem: Test ok", get_time()); 
                          $fclose(fd); 
                          $stop; end
          default : begin $display("Unexpected GPIO: 0x%0h", gpio_s); 
                          $fdisplay(fd,"%s - Load I-Mem: Test fail", get_time()); 
                          $fclose(fd); 
                          $stop;  end
      endcase
    end // cs_s
...
//------------------------------------------
  ...
  begin
    ...
    tdi_s = 0; tms_s = 0; #(2*clk_2_t); // Shift-IR (dummy: parallel load absorbs this slot)
    tdi_s = ir[7]; tms_s = 0; #(2*clk_2_t); // Shift-IR bit 7
    tdi_s = ir[6]; tms_s = 0; #(2*clk_2_t); // Shift-IR bit 6
    tdi_s = ir[5]; tms_s = 0; #(2*clk_2_t); // Shift-IR bit 5
    tdi_s = ir[4]; tms_s = 0; #(2*clk_2_t); // Shift-IR bit 4
    tdi_s = ir[3]; tms_s = 0; #(2*clk_2_t); // Shift-IR bit 3
    tdi_s = ir[2]; tms_s = 0; #(2*clk_2_t); // Shift-IR bit 2
    tdi_s = ir[1]; tms_s = 0; #(2*clk_2_t); // Shift-IR bit 1
    tdi_s = ir[0]; tms_s = 1; #(2*clk_2_t); // Exit1-IR bit 0
    tdi_s = 0; tms_s = 1; #(2*clk_2_t); // Update-IR
    tdi_s = 0; tms_s = 0; #(2*clk_2_t); // Run-Test-Idle
  end
