...
%%
%% Describes both chip variants (Variant A: no-cache with on-chip memories;
%% Variant B: I-Cache + D-Cache + Scratchpad + NOR Flash via QSPI),
%% the two-level cache hierarchy, tag/data array organisation, replacement
%% policy (PLRU), cache write policy (read-allocate, no-write; Flash is RO),
%% the CPU-to-Cache interface, the AXI4 bus to the Memory Arbiter, and
%% the SRAM macro selection.
%%
%% Status: v0.2 -- chip variant table added; WB+WA label corrected to
%%                 read-allocate/no-write; stale "superseded" note removed.
%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Chip Variants}
\label{subsec:chipVariants}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The \aschip project comprises two distinct chip variants that share the same
CPU core but differ in their memory subsystem:

\begin{table}[H]
\caption{Chip Variant Comparison}
\label{tab:asChipVariants}
\centering
\begin{tabularx}{\textwidth}{|l |X |X|}
  \hline
   & \textbf{Variant~A -- No-Cache} & \textbf{Variant~B -- Cache} \\
  \hline
  \hline
  RTL top & \texttt{RV\_NoPipeline} & \texttt{RV\_NoPipelineCache} \\
  \hline
  Instruction memory
    & On-chip I-Mem (32\,KB), loaded via JTAG scan chain; no external Flash
    & No on-chip I-Mem; instructions fetched from off-chip NOR Flash via QSPI, cached in I-Cache (4\,KB) \\
  \hline
  Data memory
    & On-chip D-Mem (8\,KB), connected via Wishbone
    & No on-chip D-Mem; read-only data cached in D-Cache (4\,KB); read-write data in on-chip Scratchpad SRAM (8\,KB) \\
  \hline
  External Flash
    & None
    & NOR Flash via QSPI (code + read-only data) \\
  \hline
  Cache subsystem
    & None
    & I-Cache + D-Cache + Memory Arbiter + Scratchpad \\
  \hline
  Peripheral bus
    & Wishbone (I-Mem, D-Mem, GPIO, CGU)
    & Wishbone (GPIO, CGU, QSPI ctrl); cache uses AXI4 \\
  \hline
  Design goal
    & Minimal area; standalone execution from on-chip memory
    & Higher performance via cache; runs from off-chip Flash, writable Scratchpad \\
  \hline
\end{tabularx}
\end{table}

\noindent
The remainder of this section (Sections~\ref{subsec:memCacheOverview}
through the end) describes Variant~B.
Variant~A does not use any cache and is described in
Section~\ref{subsec:noCacheVariant}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Variant A -- No-Cache Memory Subsystem}
\label{subsec:noCacheVariant}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Variant~A implements a simple Harvard memory subsystem with two on-chip SRAM
blocks and no cache.

\begin{itemize}
  \item \textbf{Instruction Memory (I-Mem, 32\,KB):}
    A single-port synchronous SRAM containing 8\,192 RISC-V 32-bit
    instructions (word-addressed).
    The CPU accesses I-Mem via a legacy flat bus.
    At reset, I-Mem is empty; the content is loaded via the JTAG scan chain
    (a dedicated instruction-memory shift register in the TAP).
    I-Mem is read-only during normal CPU execution.

  \item \textbf{Data Memory (D-Mem, 8\,KB):}
    A single-port synchronous SRAM with a Wishbone~B3 slave interface.
    Supports byte, halfword, word, and doubleword accesses controlled by the
    Wishbone \texttt{SEL} byte-enable signal.
    Holds \texttt{.data}, \texttt{.bss}, stack, and heap.
    Address range: \texttt{0x0000\_0000}--\texttt{0x0000\_1FFF}
    (8\,KB, mapped at base address 0).

  \item \textbf{Peripheral Bus:}
    All CPU data accesses are routed through a Wishbone master BPI.
    Address decoding selects D-Mem, GPIO, CGU, or QSPI controller by chip
    select (\texttt{csx\_s}).
\end{itemize}

There is no cache miss penalty in Variant~A: every fetch or load/store
completes in a fixed number of Wishbone cycles.
The trade-off is that all code and data must fit in the two 32\,KB + 8\,KB
on-chip memories.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Variant B -- Cache Memory Subsystem: Overview}
\label{subsec:memCacheOverview}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
NOR Flash is \textbf{read-only} from the cache's perspective: the QSPI
controller provides a cache-miss-driven read path only; there is no
byte- or word-level write path to Flash.
Accordingly the D-Cache is \textbf{read-allocate, no-write}: only load misses
trigger cache-line fills from Flash; stores to the Flash address region cause a
bus error (\texttt{dc\_err\_o}).
Because no dirty line can ever exist, no dirty bit and no write-back FSM state
are required.
...
\begin{tikzpicture}[x=1cm, y=1cm]
  ...
  \node[align=center,font=\small] at (9.0,8.95) {4-Way SA, 32\,B line, Read-alloc (Flash RO)};
...
\begin{itemize}
  \item The D-Cache metadata (Table~\ref{tab:asDCacheTag}) does \textbf{not}
    contain a dirty bit; per-way metadata is $(1+22)=23$ bits
    (valid + tag only, no dirty field).
...
\textbf{Note -- AW, W, B channels not implemented:}
The D-Cache is read-allocate with no write-back (Flash is read-only; no dirty bit exists).
The AW, W, and B channel tables below (Tables~\ref{tab:asAxi4AwCh}--\ref{tab:asAxi4BCh})
and the write-burst timing figure (Fig.~\ref{fig:asAxi4Wb}) are retained for
reference only and must \textbf{not} be implemented in \texttt{asDCacheCtrl}.
The D-Cache AXI4 master uses AR and R channels exclusively.

\begin{table}[H]
\caption{AXI4 AW Channel (Write Address) -- \textbf{NOT USED}; retained for reference only}
...
\begin{tabularx}{\textwidth}{|l |l |l |X|}
  ...
  \hline
  \texttt{awaddr}  & M$\to$S & 32 & Start address of cache-line burst; 32-byte aligned \\
  \hline
...
\begin{table}[H]
\caption{AXI4 W Channel (Write Data) -- \textbf{NOT USED}; retained for reference only}
\label{tab:asAxi4WCh}
\centering
\begin{tabularx}{\textwidth}{|l |l |l |X|}
  \hline
  ...
  \hline
  \hline
  \texttt{wdata}  & M$\to$S & 64 & One 64-bit beat of the cache line \\
  \hline
...
\begin{table}[H]
\caption{AXI4 B Channel (Write Response) -- \textbf{NOT USED}; retained for reference only}
\label{tab:asAxi4BCh}
\centering
\begin{tabularx}{\textwidth}{|l |l |l |X|}
  \hline
...
\subsubsection{Cache-Line Write-Back Transaction -- \textbf{NOT IMPLEMENTED}}
\label{subsubsec:writeback}

\textbf{This section is retained for reference only.}
The current D-Cache design has no write-back path (AW/W/B channels
eliminated; see Section~\ref{subsec:memCacheOverview}).

Figure~\ref{fig:asAxi4Wb} shows how a 4-beat INCR write burst would look
if write-back were added in a future revision.
...
\begin{tabularx}{\textwidth}{|l |l |X|}
  ...
  \hline
  \hline
  1 (highest) & D-Cache & Data miss (read-only; no write-back); stalls the entire pipeline \\
  \hline
